...
CHUNKED_CSV_PATH = r'C:\Users\Administrator\Documents\Search_engine\data\chunked_50000_v2.csv'
    ...
    try:
        ...
        for _, row in df_articles.iterrows():
            cur.execute(
                ...
                (str(row['id']), row['text'], row['domain'])
            )
        ...
        for _, row in df_chunks.iterrows():
            cur.execute(
                ...
                (str(row['chunk_id']), str(row['doc_id']), row['content'], row['domain'])
            )
